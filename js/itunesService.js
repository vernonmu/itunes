angular.module('itunes').service('itunesService', function($http, $q){
  ...
  //You can return the http request or you can make your own promise in order to manipulate the data before you resolve it.
    ...
    this.getSongData = function(artist) {
      return $http.jsonp('https://itunes.apple.com/search?term=' + artist + '&callback=JSON_CALLBACK', {data: 'callback'})
      .then(function(response){
        ...
        for (var i = 0; i <responseArray.length; i++) {
          var tempObj = {
            AlbumArt: responseArray[i].artworkUrl100,
